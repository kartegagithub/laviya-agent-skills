...
export function registerGetMyWorkTool(deps) {
    deps.server.registerTool("laviya_get_my_work", {
        ...
        return executeTool("laviya_get_my_work", deps.logger, async () => {
            const result = await getMyWork(deps.client, deps.runtimeConfig, deps.executionPolicyManager, deps.logger, parsed);
