...
export function registerOrchestratorPromptAssets(deps: RegisterOrchestratorPromptAssetsDeps): void {
  deps.server.registerResource(
    ORCHESTRATOR_PROMPT_RESOURCE_NAME,
    ORCHESTRATOR_PROMPT_RESOURCE_URI,
    {
      title: "Laviya Orchestrator System Prompt Resource",
      description:
        "Resolved orchestration system prompt from runtime config (base prompt plus optional project override).",
  ...
  deps.server.registerPrompt(
    ORCHESTRATOR_PROMPT_NAME,
    {
      ...
      description:
        "Returns the resolved orchestration system prompt loaded by this runtime instance."
    },
    async () => ({
      messages: [
        {
